...
TEST (FindNodeByPathnameVisitor, FileSystemTestFolder){
      Node * test_data = new Folder ("./test_data");
...
TEST (FindNodeByPathnameVisitor, FileSystemTestFileInNestedFolder){
      Node * test_data = new Folder("./test_data");
      Node * folder = new Folder("./test_data/folder");
      test_data->add(folder);
      Node * a_out = new File("./test_data/folder/a.out");
      folder->add(a_out);
      std::vector<std::string>* names = new std::vector<std::string>{std::string("a_out")};
      // FindNodeByPathnameVisitor fv(names);
      NodeVisitor * fv = new FindNodeByPathnameVisitor(names);
      test_data->accept(fv);
      ASSERT_EQ(a_out, fv->getNode());
      ASSERT_EQ(folder, a_out->getParent());
}
